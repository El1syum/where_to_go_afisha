...
export async function runAdmitadImport(localFilePath?: string) {
  ...
  try {
    ...
    for (let i = 0; i < collected.length; i += config.admitadFeed.batchSize) {
      const batch = collected.slice(i, i + config.admitadFeed.batchSize);
    ...
    if (seenExternalIds.size > 50) {
      const activeEvents = await prisma.event.findMany({
        where: { source: "ADMITAD", isActive: true },
      ...
      if (toDeactivate.length > 0) {
        ...
        logger.info(`Admitad: deactivated ${toDeactivate.length} events no longer in feed`);
      }
    }
