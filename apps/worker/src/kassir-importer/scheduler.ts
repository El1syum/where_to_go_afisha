...
import { parseKassirStream } from "./parser.js";
import { transformKassirOffer } from "./transformer.js";
import { upsertBatch, resetCaches } from "./upserter.js";
import type { TransformedEvent } from "../xml-importer/transformer.js";

export async function runKassirImport(localFilePath?: string) {
  ...
  if (running) {
    logger.warn("Another Kassir import is already running, skipping");
    return;
  }

  const importLog = await prisma.importLog.create({
    data: { source: "EXTERNAL_API", status: "RUNNING" },
  });
  ...
  try {
    ...
    if (localFilePath) {
      ...
      const { stream, cleanup } = await fetchXmlStream(config.kassirFeed.url);
      inputStream = stream;
      cleanupFn = cleanup;
    }
